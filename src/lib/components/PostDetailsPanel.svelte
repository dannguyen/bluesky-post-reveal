...
<section class="panel post-panel">
  ...
  <div class="metric-row">
    <span>Replies {formatCount(post.replyCount)}</span>
    <span>Reposts {formatCount(post.repostCount)}</span>
    <span>Quotes {formatCount(post.quoteCount)}</span>
    <span>Likes {formatCount(post.likeCount)}</span>
  </div>
